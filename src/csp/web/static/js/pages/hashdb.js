...
function renderHashDB({ apiGet, apiPost, el, showFlash, modal, formGroup }) {
  ...
  function renderLookup(res) {
    ...
    if (res.label) div.appendChild(el('p', { style: 'margin-top:4px;font-size:13px;' }, `Label: ${res.label}`));
    if (res.source) div.appendChild(el('p', { style: 'margin-top:4px;font-size:13px;' }, `Source: ${res.source}`));
  }
  ...
  async function loadSources() {
    try {
      ...
      div.innerHTML = '';
      if (sources.length === 0) {
        div.appendChild(el('p', { style: 'color:var(--text2);font-size:13px;' }, 'No imports yet.'));
        return;
      }
      sources.forEach(s => {
        div.appendChild(el('div', { style: 'font-size:13px;padding:4px 0;border-bottom:1px solid var(--border);' },
