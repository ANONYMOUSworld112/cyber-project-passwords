...
function renderLogs({ apiGet, apiPost, el, showFlash, modal, formGroup }) {
  ...
  function renderAnalyzeResult(r) {
    ...
    div.innerHTML = '';
    div.appendChild(el('div', { style: 'font-size:13px;' },
      el('p', null, `Format: ${r.format}`),
      el('p', null, `Lines: ${r.lines}`),
      el('p', null, `Run ID: ${r.id}`)));

    if (r.levels && Object.keys(r.levels).length > 0) {
      div.appendChild(el('h3', { style: 'margin-top:12px;font-size:13px;color:var(--text2);' }, 'Levels:'));
      Object.entries(r.levels).forEach(([k, v]) => {
        div.appendChild(el('div', { style: 'padding-left:12px;font-size:12px;' }, `${k}: ${v}`));
      });
    }

    if (r.top_patterns && r.top_patterns.length > 0) {
      div.appendChild(el('h3', { style: 'margin-top:12px;font-size:13px;color:var(--text2);' }, 'Top patterns:'));
      r.top_patterns.slice(0, 10).forEach(p => {
        div.appendChild(el('div', { style: 'padding-left:12px;font-size:12px;' }, `${String(p.count).padStart(5)}  ${(p.pattern || '').slice(0, 120)}`));
      });
    }

    if (r.error_lines && r.error_lines.length > 0) {
      div.appendChild(el('h3', { style: 'margin-top:12px;font-size:13px;color:var(--red);' }, 'Error lines:'));
      r.error_lines.slice(0, 10).forEach(l => {
        div.appendChild(el('div', { style: 'padding-left:12px;font-size:11px;font-family:monospace;color:var(--red);white-space:pre-wrap;word-break:break-all;' }, l));
      });
    }
  }
  ...
  async function loadRuns() {
    try {
      ...
      div.innerHTML = '';
      if (runs.length === 0) {
        div.appendChild(el('p', { style: 'color:var(--text2);font-size:13px;' }, 'No runs yet.'));
        return;
      }
      const tbl = el('table');
    ...
    const m = modal('Run details', body => {
      ['id', 'path', 'format', 'lines', 'analyzed_at'].forEach(f => {
        body.appendChild(el('div', { className: 'form-group' },
          el('label', null, f),
          el('div', { style: 'padding:8px 0;font-size:13px;' }, String(r[f] || ''))));
      });
      if (r.levels) {
        body.appendChild(el('h3', { style: 'margin-top:12px;font-size:13px;' }, 'Levels:'));
        Object.entries(r.levels).forEach(([k, v]) => {
          body.appendChild(el('div', { style: 'padding-left:12px;font-size:12px;' }, `${k}: ${v}`));
        });
      }
    }, actions => {
