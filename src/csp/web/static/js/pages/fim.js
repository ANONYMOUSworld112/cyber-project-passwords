...
function renderFIM({ apiGet, apiPost, apiDelete, el, showFlash, modal, formGroup }) {
  ...
  resultCard.appendChild(el('div', { id: 'fim-result-content', style: 'color:var(--text2);font-size:20px;' }, 'Run a baseline or scan to see results.'));
  app.appendChild(resultCard);

  function renderWatches(paths) {
    watchList.innerHTML = '';
    if (paths.length === 0) {
      watchList.appendChild(el('p', { style: 'color:var(--text2);font-size:20px;' }, 'No watched paths.'));
      return;
    }
    paths.forEach(p => {
      ...
      row.appendChild(el('span', { style: 'font-size:20px;' }, p));
  ...
  function renderResult(data) {
    ...
    if (data.scans) {
      data.scans.forEach(s => {
        content.appendChild(el('h3', { style: 'margin:12px 0 8px;font-size:22px;' }, s.root));
        content.appendChild(el('p', { style: 'font-size:20px;' },
          `Added: ${s.added.length} | Removed: ${s.removed.length} | Changed: ${s.changed.length}`));
        if (s.added.length > 0) {
          content.appendChild(el('div', { style: 'margin-top:6px;' }, el('strong', { style: 'color:var(--c-pure-white);' }, '[+] Added:')));
          s.added.slice(0, 20).forEach(f =>
            content.appendChild(el('div', { style: 'font-size:18px;color:var(--c-pure-white);padding-left:12px;font-family:var(--font-mono);' }, `+ ${f}`)));
        }
        if (s.removed.length > 0) {
          content.appendChild(el('div', { style: 'margin-top:6px;' }, el('strong', { style: 'color:var(--c-silver-gray);' }, '[-] Removed:')));
          s.removed.slice(0, 20).forEach(f =>
            content.appendChild(el('div', { style: 'font-size:18px;color:var(--c-silver-gray);padding-left:12px;font-family:var(--font-mono);text-decoration:line-through;' }, `- ${f}`)));
        }
        if (s.changed.length > 0) {
          content.appendChild(el('div', { style: 'margin-top:6px;' }, el('strong', { style: 'color:var(--c-silver-gray);' }, '[*] Modified:')));
          s.changed.slice(0, 20).forEach(f =>
            content.appendChild(el('div', { style: 'font-size:18px;color:var(--c-silver-gray);padding-left:12px;font-family:var(--font-mono);' }, `* ${f}`)));
        }
      });
    } else if (data.root) {
      content.appendChild(el('p', { style: 'font-size:20px;' },
