...
function renderLogin({ apiGet, apiPost, el, showFlash }) {
  ...
  const card = el('div', { className: 'card', style: 'max-width:540px;margin:60px auto;border:1px solid var(--c-silver-gray);box-shadow:0 16px 40px rgba(0,0,0,0.9), 0 0 25px rgba(255,255,255,0.08);' });
  const header = el('div', { style: 'text-align:center;margin-bottom:28px;' });
  header.appendChild(el('div', { style: 'font-family:var(--font-display);font-size:34px;font-weight:700;color:var(--c-pure-white);letter-spacing:2px;' }, '[ CYBERBLACK :: AUTH ]'));
  header.appendChild(el('div', { style: 'font-family:var(--font-display);font-size:18px;color:var(--c-silver-gray);margin-top:6px;letter-spacing:1px;' }, '● OFFLINE ENCRYPTED TERMINAL'));
  header.appendChild(el('div', { style: 'font-size:16px;color:var(--c-mid-gray);margin-top:4px;letter-spacing:0.5px;' }, 'AES-256-GCM // ARGON2ID KEY DERIVATION'));
  ...
  const pwInput = el('input', { type: 'password', id: 'login-pw', placeholder: 'Enter master password' });
  form.appendChild(el('div', { className: 'form-group' },
    el('label', null, 'Password'), pwInput));

  const submitBtn = el('button', { className: 'btn btn-primary', style: 'width:100%;padding:14px;font-weight:700;' }, 'AUTHENTICATE & UNLOCK');
  form.appendChild(submitBtn);

  const recoverLink = el('a', { href: '#/recovery', style: 'display:block;text-align:center;margin-top:16px;font-size:20px;color:var(--c-silver-gray);' }, 'Forgot password?');
  form.appendChild(recoverLink);

  const resetLink = el('a', { href: '#', id: 'reset-link', style: 'display:block;text-align:center;margin-top:10px;font-size:18px;color:var(--c-mid-gray);' }, 'Reset all data (factory reset)');
  ...
  document.getElementById('reset-link').addEventListener('click', e => {
    ...
    m.appendChild(el('p', { style: 'margin-bottom:12px;font-size:20px;' },
    ...
    confirmBtn.addEventListener('click', async () => {
      ...
      try {
        await apiPost('/api/reset', { confirm: 'RESET' });
        overlay.remove();
