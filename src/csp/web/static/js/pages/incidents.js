...
function renderIncidents({ apiGet, apiPost, apiPut, el, showFlash, modal, formGroup }) {
  ...
  const filterLabel = el('label', { style: 'display:flex;align-items:center;gap:6px;font-size:13px;' });
  ...
  function renderTable(data) {
    tableWrap.innerHTML = '';
    if (data.length === 0) {
      tableWrap.appendChild(el('p', { style: 'color:var(--text2);padding:20px;' }, 'No incidents.'));
      return;
    }
    const tbl = el('table');
