...
function renderVault({ apiGet, apiPost, apiPut, apiDelete, el, showFlash, modal, formGroup }) {
  ...
  const showPwLabel = el('label', { style: 'display:flex;align-items:center;gap:6px;font-size:13px;white-space:nowrap;' });
  ...
  function renderTable(data) {
    tableWrap.innerHTML = '';
    if (data.length === 0) {
      tableWrap.appendChild(el('p', { style: 'color:var(--text2);padding:20px;' }, 'No credentials yet.'));
      return;
    }
    const tbl = el('table');
