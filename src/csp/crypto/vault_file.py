...
class VaultFile:
    ...
    def from_json(cls, text: str) -> "VaultFile":
        ...
        try:
            ...
            raise CorruptVaultError(f"invalid envelope version: {e}") from e
        if v != _FILE_VERSION:
...
def write_vault_file(target_path, payload: Any, kdf: KDFParams, key: bytes) -> None:
    ...
    aad = _aad(env_no_ct)
    blob = aead_encrypt(key, plaintext, aad=aad)
    vf = VaultFile(kdf=kdf, blob=blob)
    atomic_write_text(target_path, vf.to_json())
...
def read_vault_file(target_path, key: bytes) -> Any:
    ...
    aad = _aad(env_no_ct)
    try:
        plaintext = aead_decrypt(key, vf.blob, aad=aad)
    except InvalidTag as e:
        raise CorruptVaultError("authentication failed (wrong key or tampered file)") from e
    except ValueError as e:
        raise CorruptVaultError(f"invalid ciphertext blob: {e}") from e
    try:
...
def make_vault_envelope(kdf: KDFParams, payload: Any, key: bytes) -> VaultFile:
    ...
    aad = _aad(env_no_ct)
    blob = aead_encrypt(key, plaintext, aad=aad)
    return VaultFile(kdf=kdf, blob=blob)
