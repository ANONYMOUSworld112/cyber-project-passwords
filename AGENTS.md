...
pip install argon2-cffi cryptography platformdirs pytest
...
All tests redirect `csp.paths.data_dir()` to a temporary directory via
the `CSP_DATA_DIR` environment variable (see `tests/conftest.py`). No
test ever touches the user's real data directory.
